...
#
# The server's systemd timer (portfolio-deploy.timer) runs this every couple of
# minutes with DEPLOY_CHECK_DIGEST=1; it deploys only when the published
# :latest image digest differs from the one last deployed. Keeping the trigger
# outbound means the box's CrowdSec IP filtering can never block a deploy the
# way an inbound SSH from a GitHub runner could.
#
# It first syncs itself and docker-compose.prod.yml from the public repo at the
# requested ref, then re-execs the fresh copy to roll out.

set -euo pipefail

cd "$(dirname "$(realpath "$0")")"

REPO="jigar-dhulla/portfolio"
